 ...
 *   isPalindrome('No leomn, no melon') => true
 */
...
function findLongestWord(sentence) {
  ...
  for (let wordId = 0; wordId < words.length; wordId += 1) {
    if (longestWord.length < words[wordId].length) longestWord = words[wordId];
  }
  return longestWord;
}
...
function reverseWords(str) {
  ...
  for (let wordId = 0; wordId < words.length; wordId += 1) {
    words[wordId] = words[wordId].split('').reverse().join('');
  }
  return words.join(' ');
}
...
function invertCase(str) {
  ...
  let myStr = '';
  for (let wordId = 0; wordId < words.length; wordId += 1) {
    for (let charId = 0; charId < words[wordId].length; charId += 1) {
      if (words[wordId][charId] === words[wordId][charId].toUpperCase()) {
        myStr += words[wordId][charId].toLowerCase();
      } else myStr += words[wordId][charId].toUpperCase();
    }
    if (wordId !== words.length - 1) myStr += ' ';
  }
  return myStr;
}
...
function extractNameFromTemplate(value) {
  return value.split(', ').slice(1).toString().slice(0, -1);
}
